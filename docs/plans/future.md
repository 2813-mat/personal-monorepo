...
## ~~T2 — Tela: Cartões (`/cards`)~~ ✅ concluída
...
## ~~T3 — Tela: Orçamentos (`/budgets`)~~ ✅ concluída
...
## ~~T4 — Tela: Gastos fixos (`/fixed`)~~ ✅ concluída
...
_Pendentes_ — itens de `MOCK_FIXED` sem correspondência em `MOCK_TRANSACTIONS.recurring`:
...
_Pagos este mês_ — transações com `recurring: true`:

- Mesmas colunas + data de pagamento

---

## ~~T5 — Tela: Metas~~ (`/goals`) ✅ concluída
